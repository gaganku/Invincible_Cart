...
// Email Service
const { sendOTPEmail } = require('../../src_backend/utils/emailService');
...
app.post('/api/login', async (req, res) => {
    try {
        ...
        if (user.password !== password) {
            console.log(`[Auth] Invalid password. Expected: ${user.password}, Got: ${password}`);
            return res.status(401).json({ error: 'Invalid credentials' });
        }

        console.log('[Auth] Password matched. Generating OTP...');
...
const googleCallbackHandler = (req, res, next) => {
    console.log(`[Auth] Google Callback hit: ${req.url}`);
    passport.authenticate('google', async (err, user, info) => {
        ...
        const frontendUrl = process.env.NODE_ENV === 'production' 
            ? process.env.BASE_URL || 'https://shopping-cart-prod.vercel.app'
            : 'http://localhost:5173';
...
// Verify OTP (Generic 2FA)
app.post('/api/auth/verify-2fa', async (req, res) => {
    try {
        const { otp } = req.body;
        const sessionData = req.session.googleAuth; // We reuse this session key for now
        ...
        if (sessionData.userId) {
            ...
            req.login(user, (err) => {
                ...
                delete req.session.googleAuth;
                req.session.save(() => {
                    res.json({ message: 'Verification successful', username: user.username, redirect: '/index.html' });
                });
            });
        } else {
